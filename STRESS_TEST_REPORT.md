...
1. **客户端重试**: 对"瓶位繁忙"错误增加 2-3 次指数退避重试，可将成功率提升至 99.9%+
2. **瓶位锁优化**: 当前 `lockWithTimeout` 等待 3 秒，高并发下 p99 达 3.1 秒。可考虑缩短锁持有时间或增加瓶位数量
3. **create 阶段延迟**: p95=1.8s, p99=3.1s，主要为锁等待。非功能缺陷，为并发控制的正常代价
4. **容量回补幂等**: `restoreCapacity` 使用 `LEAST(initial_capacity, current + ml)` 保证不会超补，支付失败返还 + 超时关闭返还的双重回补是安全的

## 7. 结论

- **数据一致性**: ✅ 10万单库存零差异，无超卖
- **并发安全**: ✅ Redis分布式锁 + DB行锁双重保护，瓶位繁忙错误率1.45%
- **状态流转**: ✅ 所有成功订单完整流转到 COMPLETED
- **Bug修复**: ✅ 支付失败容量回补问题已修复并验证
- **系统稳定性**: ✅ 10万级高并发下无宕机、无数据错乱
